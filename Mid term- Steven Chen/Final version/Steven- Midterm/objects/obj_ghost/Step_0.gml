/// @description Control of the ghost, setting of the timer
...
//if the ghost was hit by the bullet
if place_meeting(x + h_speed, y, obj_bullet){
   while !place_meeting(x + sign(h_speed), y, obj_bullet){
         x += sign (h_speed);
   }
   h_speed = 0;
}
...
//if the ghost was hit by the bullet
if place_meeting(x + v_speed, y, obj_bullet){
   while !place_meeting(x + sign(v_speed), y, obj_bullet){
         x += sign (v_speed);
   }
   v_speed = 0;
}
...
global.frames--;//step is the frame of the game: 60

if (global.frames <= 0){
  audio_play_sound(sound_humanlaugh,1,0);
  room_goto(room_Human);
}
